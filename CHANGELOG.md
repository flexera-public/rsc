...
* Add support for using Flexera One refresh tokens for authentication.

v8.1.0 / 2020-06-18
-------------------

* Add support for Credentials.
* Add support for selectable actions in Policies.
...
* Fix bug introduced with storing refresh token in `~/.rsc` where it took precedence over the `-r`/`--refresh-token`
  flag rather than the flag taking precedence over the config file value like it should.
